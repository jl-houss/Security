...
import pyperclip
from assets.code.logic import center, clear, password_decrypt, password_encrypt
...
class Authentifiant:
    ...
    def check(self, search):
        return any(
            map(
                ...
                [self.title, self.domain, self.username, self.email, self.category],
            )
        )
    ...
    def insert(self):
        ...
        curr.execute(
            "INSERT INTO Authentifiants (parentId, title, domain, username, email, password, category, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (
                self.parentId,
                self.title,
                self.domain,
    ...
    def get(authId) -> super:
        ...
        (
            ...
            *rest,
            ...
            "SELECT * FROM Authentifiants WHERE authId=?", (authId,)
        ).fetchone()
        ...
        return Authentifiant(
            authId, parentId, title, domain, username, email, password, *rest
        )

    @staticmethod
    def generate(uppers: bool, numbers: bool, symbols: bool, len: int):

        all_chars = string.ascii_letters if uppers else string.ascii_letters.lower()
...
class AuthentifiantsTable(CTkFrame):
    def __init__(self, page: Authentifiants):
        ...
        self.style.configure("Treeview", rowheight=50, font=Fonts().EntryFont)

        self.style.configure(
            "Treeview.Heading", foreground=Colors.Teal, font=Fonts().NavButtonFont
        )

        self.style.configure("Treeview.Cell", foreground="red")

        self.style.map("Treeview", background=[("selected", Colors.Teal)])
    ...
    def view(self):
        ...
        self.vault.bind(
            "<Button-1>",
            lambda event: self.Table.selection_remove(*self.Table.selection()),
        )
    ...
    def search(self):
        ...
        records = curr.execute(
            f"SELECT * FROM Authentifiants WHERE parentId = ?", (env["USERID"])
        ).fetchall()
        ...
        for i, record in enumerate(records):
            ...
            if auth.check(self.SearchEntry.get()):
                values = (
                    auth.title,
                    auth.username if auth.username else auth.email,
                    auth.category,
                    "",
                    "...",
                )
    ...
    def action(self, event: Event, action: str):
        ...
        elif region == "cell" and (
            (action == "<Button-1>" and event.x >= 996) or action == "<Button-3>"
        ):
...
class EditPassword(CTkToplevel):
    ...
    def view(self):
        CTkLabel(
            ...
            height=150,
            fg_color=Colors.Mirage,
            text=f"{'Modifier' if self.authId else 'Ajouter'} un identifiant",
            font=CTkFont("Roboto", 35, "bold"),
        ).pack(fill="x")

        CTkLabel(
            self,
            text="Titre:",
            font=CTkFont("Roboto", 18, "bold"),
            text_color=Colors.Black,
            anchor="e",
            width=170,
            height=40,
    ...
    def save(self):
        ...
        auth = Authentifiant(
            self.authId, None, title, domain, username, email, password, category, note
        )
        if self.authId is not None:
            auth.update()
        else:
...
class GeneratePassword(CTkToplevel):
    ...
    def view(self):
        ...
        self.LengthSlider.bind(
            "<ButtonRelease-1>",
            lambda event: [
                self.LengthBox.configure(
                    textvariable=StringVar(value=str(int(self.LengthSlider.get())))
                ),
                self.generate(),
            ],
        )
    ...
    def generate(self):
        ...
        self.SecurityLevel.configure(
            text=strength["text"], text_color=strength["color"]
        )
...
class ActionPopup(Menu):
    ...
    def copy_password(self):
        ...
        password = curr.execute(
            f"SELECT password FROM Authentifiants WHERE authId={self.tags[0]}"
        ).fetchone()[0]
    ...
    def copy_login(self):
        conn = sqlite3.connect(env["DB"])
        curr = conn.cursor()
        username, email = curr.execute(
            f"SELECT username, email FROM Authentifiants WHERE authId={self.tags[0]}"
        ).fetchone()
        conn.close()
        ...
        if username and not username.isspace():
            ...
            ToastNotifier().show_toast(
                "Security",
                "Le nom d'utilisateur a été copié dans la presse papier",
                duration=3,
                threaded=True,
            )
        else:
            ...
            ToastNotifier().show_toast(
                "Security",
                "L'email a été copié dans la presse papier",
                duration=3,
                threaded=True,
            )
